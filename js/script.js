...
var octoPrint = {
	"counter":0,
	"reception":["aon1","aon2","aon3","aon4","aon5","aon6","aon7","aon8","aon9","nv1","nv2","nv3","nv4"],
...
var checkPrinterStatus = function (){
	var url, divId;
	if(!!octoPrint && octoPrint.reception.length){
		if(octoPrint.counter < octoPrint.reception.length){
			divId = octoPrint.reception[octoPrint.counter];
			url = octoPrint[divId];
...
var loop = function (counter){
	octoPrint.counter++;
	if(octoPrint.counter < octoPrint.reception.length){
		//not down to the last printer yet.
		checkPrinterStatus();
	}
	else{
		//the last printer status has been updated 
		octoPrint.counter = 0;
		setTimeout(checkPrinterStatus,60000); //pause for 60secs and loop again.
	}
}
